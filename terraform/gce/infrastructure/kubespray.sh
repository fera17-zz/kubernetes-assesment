...
if [ -z "$bastion_ip" ]; then
  ...
  nat_master_ips=$(gcloud compute instances list --filter="${prefix}-master" --format=json | jq -r '.[].networkInterfaces[].accessConfigs[].natIP' | tr "\n" " " | sed -e "s/ \{1,\}$//")
  node_ips=$(gcloud compute instances list --filter="${res_prefix}-worker" --format=json | jq -r '.[].networkInterfaces[].networkIP' | tr "\n" " " | sed -e "s/ \{1,\}$//")
  nat_node_ips=$(gcloud compute instances list --filter="${res_prefix}-worker" --format=json | jq -r '.[].networkInterfaces[].accessConfigs[].natIP' | tr "\n" " " | sed -e "s/ \{1,\}$//")
else
